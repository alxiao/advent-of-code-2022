...
3. Iterate through the array and collect the following as running sums:
  - The points achieved from the element played (Rock (X) = 1 pt, Paper (Y) = 2 pts, Scissors (Z) = 3 pts)
  - The points achieved from whether it's a loss (0 pts), a tie (3 pts), or a win (6 pts)
4. Add the two sums together to get the total score
*/
...
const ELEMENT_POINTS_MAP = {
  X: 1,
  Y: 2,
  Z: 3,
};

const ELEMENT_WIN_MAP = {
  X: 'C',
  Y: 'A',
  Z: 'B',
};

const ELEMENT_DRAW_MAP = {
  X: 'A',
  Y: 'B',
  Z: 'C',
};

const getOutcomeSum = (elementsPlayed) => {
  const opponent = elementsPlayed[0];
  const me = elementsPlayed[1];

  if (opponent === ELEMENT_DRAW_MAP[me]) {
    return 3;
  } else if (opponent === ELEMENT_WIN_MAP[me]) {
    return 6;
  }
...
https.get(INPUT_URL, options, (res) => {
  res.on('data', (d) => {
    ...
    input.forEach((round) => {
      ...
      const elementPlayed = elements[1];

      runningPlayedElementSum += ELEMENT_POINTS_MAP[elementPlayed];
